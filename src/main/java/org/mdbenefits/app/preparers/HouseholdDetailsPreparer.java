    ...
    public Map<String, SubmissionField> prepareSubmissionFields(Submission submission, PdfMap pdfMap) {
        ...
        if (household != null) {
            for (int i = 0; i < household.size(); i++) {
                ...
                results.put("householdHighestEducation" + i, new SingleField("householdHighestEducationFormatted",
                        SubmissionUtilities.PDF_EDUCATION_MAP.get(educationStatus), i + 1));

                var maritalStatus = householdMember.get("householdMemberMaritalStatus");
                results.put("householdMaritalStatus" + i, new SingleField("householdMaritalStatusFormatted",
                        SubmissionUtilities.PDF_MARITAL_STATUS_MAP.get(maritalStatus), i + 1));

                var relationshipStatus = householdMember.get("householdMemberRelationship");
                results.put("householdRelationship" + i, new SingleField("householdRelationshipFormatted",
                        SubmissionUtilities.PDF_RELATIONSHIP_MAP.get(relationshipStatus), i + 1));
                ...
                results.put("householdUSCitizen" + i, new SingleField("householdUSCitizenDerived",
                        nonCitizens != null && nonCitizens.contains(uuid) ? "No" : "Yes", i + 1));
