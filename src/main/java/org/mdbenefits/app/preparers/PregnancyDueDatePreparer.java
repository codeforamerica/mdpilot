    ...
    public Map<String, SubmissionField> prepareSubmissionFields(Submission submission, PdfMap pdfMap) {
        ...
        if (!pregnancies.isEmpty()) {
            ...
            for (var uuid : pregnancies) {
                ...
                var pregnantMemberName = SubmissionUtilities.getHouseholdMemberFullnameByUUID((String) uuid,
                        submission.getInputData());
                pregnantMemberNames.add(pregnantMemberName);
            }
            result.put("pregnantMemberNames",
                    new SingleField("pregnantMemberNames", Strings.join(pregnantMemberNames, ','), null));
            result.put("pregnantMemberDueDates",
                    new SingleField("pregnantMemberDueDates", Strings.join(pregnantMemberDueDates, ','), null));
        }
