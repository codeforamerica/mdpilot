    ...
    public Map<String, SubmissionField> prepareSubmissionFields(Submission submission, PdfMap pdfMap) {
        ...
        if (!students.isEmpty()) {
            ...
            for (var studentUUID : students) {
                var studentName = SubmissionUtilities.getHouseholdMemberFullnameByUUID((String) studentUUID,
                        submission.getInputData());
                var schoolName = (String) submission.getInputData()
                        .get("schoolName%s%s".formatted(DYNAMIC_FIELD_MARKER, studentUUID));
                var attendance = (String) submission.getInputData()
                        .get("schoolEnrollmentLevel%s%s".formatted(DYNAMIC_FIELD_MARKER, studentUUID));
